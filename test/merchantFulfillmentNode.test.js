...
describe('Merchant fulfillment node', () => {
  test('construye un nodo merchant fulfillment canonico', () => {
    const result = buildMerchantFulfillmentNodeEntry({ id: 'MF-1', zona: 'centro' });

    expect(result.validation.ok).toBe(true);
    expect(result.node.tipo).toBe('merchant_fulfillment');
    expect(result.node.estado).toBe('DISPONIBLE');
    expect(result.node.capabilities).toEqual(expect.arrayContaining(['order_fulfillment']));
  });

  test('opera con el mismo fulfillment engine', () => {
    const engine = createFulfillmentEngine({ clock: () => 1000 });
    const node = buildMerchantFulfillmentNodeEntry({ id: 'MF-1' });
