...
import { obtenerMontoPedido } from './src/services/smartDispatchService.js';
import { evaluarElegibilidadPedido } from './src/services/smartDispatchService.js';
import { firstFiniteNumber, roundMoney } from './src/services/smartDispatchService.js';
...
function firstFiniteNumberLocal(...values) {
...
function aplicarReservaCapital(actual, pedidoId, monto, timestamp) {
  ...
  const reservadoActual = firstFiniteNumberLocal(
    billetera.capital_reservado,
    finanzas.capital_reservado,
    actual.capital_reservado
  ) || 0;
  const billeteraTotal = firstFiniteNumberLocal(
    actual.billetera_guerra,
    billetera.billetera_guerra,
    finanzas.billetera_guerra,
    actual.perfil?.billetera_guerra
  );
  const capitalDisponibleActual = firstFiniteNumberLocal(
    billetera.capital_disponible,
    billetera.efectivo_disponible,
    finanzas.capital_disponible,
    finanzas.efectivo_disponible
  );
  const nuevoReservado = roundMoneyLocal(reservadoActual + monto);
  const nuevoDisponible = billeteraTotal !== null
    ? Math.max(0, roundMoneyLocal(billeteraTotal - nuevoReservado))
    : (capitalDisponibleActual === null ? undefined : Math.max(0, roundMoneyLocal(capitalDisponibleActual - monto)));
...
async function run() {
  const snapshot = await db.ref(`repartidores/${uid}`).once('value');
  console.log('CURRENT DRIVER', JSON.stringify(snapshot.val(), null, 2));
  const tx = await db.ref(`repartidores/${uid}`).transaction((actual) => {
    console.log('TX callback actual type', typeof actual, 'exists', actual !== null && actual !== undefined, 'actual', JSON.stringify(actual, null, 2));
    if (!actual || typeof actual !== 'object') return;
    const reserva = getReservaCapital(actual, pedidoId);
    console.log('existing reserva', reserva);
    const elegibilidad = evaluarElegibilidadPedido(pedido, actual);
    console.log('eligibility during tx', JSON.stringify(elegibilidad, null, 2));
    if (!elegibilidad.ok) return;
    return aplicarReservaCapital(actual, pedidoId, 250, Date.now());
  }, (error, committed, snapshot2) => {
    console.log('TX callback complete error', error, 'committed', committed, 'snapshot exists', snapshot2?.exists(), 'snapshot val', snapshot2?.val && JSON.stringify(snapshot2.val(), null, 2));
  });

  console.log('TX result committed', tx.committed, 'snapshot exists', tx.snapshot.exists(), 'snapshot val', JSON.stringify(tx.snapshot.val(), null, 2));
}
