...
async function run() {
  ...
  console.log('CURRENT DRIVER', JSON.stringify(driverSnap.val(), null, 2));

  const txRes = await db.ref(`repartidores/${uid}`).transaction((actual) => {
    console.log('TX callback actual:', JSON.stringify(actual, null, 2));
  ...
  console.log('TX result committed', txRes.committed);
  if (txRes.snapshot) {
    console.log('TX snapshot exists', txRes.snapshot.exists());
    console.log('TX snapshot val', JSON.stringify(txRes.snapshot.val(), null, 2));
  }
}
