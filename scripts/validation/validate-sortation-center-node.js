...
const node = buildSortationCenterFulfillmentNode({ id: 'SC-1', zona: 'norte' });
...
if (!node.validation.ok || node.node.tipo !== 'sortation_center') {
  console.error('El nodo sortation center no cumple el contrato');
  ok = false;
}

if (!accepted.ok || accepted.order.estado_pedido !== 'EN_CURSO') {
  console.error('El engine no acepta pedidos sobre el nodo sortation center');
  ok = false;
}
...
console.log('validate-sortation-center-node: OK');
