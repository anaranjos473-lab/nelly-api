...
const node = buildPharmacyFulfillmentNode({ id: 'PH-1', zona: 'norte' });
...
if (!node.validation.ok || node.node.tipo !== 'pharmacy') {
  console.error('El nodo de farmacia no cumple el contrato');
  ok = false;
}

if (!accepted.ok || accepted.order.estado_pedido !== 'EN_CURSO') {
  console.error('El engine no acepta pedidos sobre el nodo de farmacia');
  ok = false;
}
...
console.log('validate-pharmacy-node: OK');
